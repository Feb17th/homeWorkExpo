...
type DataLocationScreenRouteProp = RouteProp<RootStackParamList, RootScreens.DATA_LOCATION>;

interface DataLocationScreenProp {
    route: DataLocationScreenRouteProp;
}


export const DataLocation = ({ route }: DataLocationScreenProp) => {

    const dataLocation = route.params.data;
    return (
        <View style={styles.container}>
            <View style={styles.containerContent}>
                <Text style={styles.headerName}>{dataLocation.name}</Text>
                <Image source={{ uri: urlImage }} style={styles.image} />
                <Text style={{ fontSize: FontSize.REGULAR, fontWeight: "500", width: "100%" }}>{i18n.t(LocalizationKey.DATALOCATION_DESCRIPTION)}:</Text>
                <Text style={styles.description}>{dataLocation.description}</Text>
                <Text style={{ fontSize: FontSize.REGULAR, fontWeight: "500", width: "100%" }}>{i18n.t(LocalizationKey.DATALOCATION_POSITION)}:</Text>
                <Text style={styles.position}>{dataLocation.position}</Text>
            </View>
            <TouchableOpacity style={styles.commentButton}>
                <Text style={{fontSize: 17, fontWeight: "500", color: "white"}}>Bình luận</Text>
            </TouchableOpacity>
        </View>
    )
}
